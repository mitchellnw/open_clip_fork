    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                uhat = u.clone().mul_(beta1hat).addcmul_(g,g,value=1.0-beta1hat)
                u = u.mul_(beta2hat).addcmul_(g,g,value=1.0-beta2hat)

                denominator = uhat.sqrt().add_(self.eps)
                ...
                rms = torch.div(
                    g.pow(2), 
                    torch.maximum(uhat, (self.eps ** 2) * torch.ones_like(uhat))
                )
