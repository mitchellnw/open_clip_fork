...
class LAdamW2(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=0.004, weight_decay=0.2, betas=(0.9, 0.999), eps=1e-6, individual_betas=False):
        ...
        super(LAdamW2, self).__init__(params, defaults)
        ...
        print('Using LAdamW2-v1')


    def __setstate__(self, state):
        super(LAdamW2, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                vhat = v.clone().mul_(beta1hat).add_(g, alpha=1.0-beta1hat)
                v = v.mul_(0.99).add_(g, alpha=1.0-0.99)
                u = u.mul_(beta2hat).addcmul_(g,g,value=1.0-beta2hat)

                denominator = u.sqrt().add_(self.eps)

                theta = theta.mul_(1.0-lr*weight_decay).addcdiv_(vhat, denominator, value=-lr)
