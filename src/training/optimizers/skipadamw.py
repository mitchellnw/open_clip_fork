...
class SkipAdamW(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr=0.004, weight_decay=0.2, betas=(0.9, 0.999), eps=1e-6):
        ...
        super(SkipAdamW, self).__init__(params, defaults)
        ...
        print('Using SkipAdamW')


    def __setstate__(self, state):
        super(SkipAdamW, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                v = v.mul(beta1hat).add_(g, alpha=1.0-beta1hat)
                u = u.mul(beta2hat).addcmul_(g,g,value=1.0-beta2hat)
                ...
                skip_step = False
                if step > 500 and rms.mean().sqrt() > 1.5:
                    print('skipping a step', step)
                    skip_step = True
                else:
