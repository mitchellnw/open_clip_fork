"""
Copied from experiment starter code.
"""
import numpy as np
import torch
import logging
...
class Averager(object):
    def __init__(self, model, method, freq, T):
        ...
        # elif sub_sample == 'log':
        #     self.freq = int(total_steps / np.log(total_steps))
        # elif sub_sample == 'sqrt':
        #     self.freq = int(np.sqrt(total_steps))
        # elif sub_sample.startswith('freq'):
        #     self.freq = int(sub_sample.split('_')[-1])
        # else:
        #     logger.error(f'Unknown sub-sampling argument: {sub_sample}')
        ...
        self.state_dict = {'update_counter': self.update_counter, 
