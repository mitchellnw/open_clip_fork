...
class LegacyDistributedDataParallel(nn.Module):
    ...
    def all_reduce_grads(self):
        ...
        def all_reduce_params(params, process_group):
            buffer = self.buffer
            ...
            all_reduce(buffer, process_group)
            ...
            for params in self.per_device_params:
                ...
                for param in params:
                    if not param.requires_grad:
                        continue
                    if hasattr(param, 'process_group'):
                        process_group = param.process_group
                    else:
                    ...
                    if hasattr(param, 'process_group'):
                        all_reduce_params([param], process_group=process_group)
                    else:
                        if sz > self.buffer.numel():
                            ...
                            if offset + sz > self.buffer.numel():
                                all_reduce_params(buffered_params,process_group=process_group)
                                offset = 0
