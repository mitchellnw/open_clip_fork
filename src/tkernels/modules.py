...
from .int8_matmul_rowwise_dequantize_bias import int8_matmul_rowwise_dequantize_bias
...
from tkernels.int8_matmul_global_dequanitze import int8_matmul_global_dequanitze, int8_matmul_global_dequanitze_bias
...
class _switchback_new(torch.autograd.Function):
    ...
    def forward(ctx, X_3D, W, bias):
        ...
        X_int8, state_X = quantize_global(X)
        W_int8, state_W = quantize_global(W)
        ctx.save_for_backward = X, W
        return int8_matmul_global_dequanitze_bias(
            X_int8, W_int8.t(), state_X, state_W, bias
        ).view(*X_3D.size()[:-1], -1)
    ...
    def backward(ctx, G_3D):
        ...
        X, W = ctx.save_for_backward
        if ctx.needs_input_grad[0]:
            G_int8, state_G = quantize_global(G)
            W_int8, state_W = quantize_global_transpose(W)
            grad_X = int8_matmul_global_dequanitze(G_int8, W_int8.t(), state_G, state_W).view(
                *G_3D.size()[:-1], -1
            )
        if ctx.needs_input_grad[1]:
            grad_W = torch.matmul(G.t(), X.to(G.dtype))
...
class SwitchBackNewLinear(nn.Linear):

    def forward(self, x):
        return _switchback_new.apply(x, self.weight, self.bias)