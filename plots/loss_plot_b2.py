...
if __name__ == '__main__':
    ...
    file_list = []
    file_list = [
        #(f'adamw-ViT-B-32-{4096*4}-2e-3-0.99-v0', 'AdamW 0.99', cmap(1.), -1),
        #(f'customadamw-ViT-B-32-{4096*4}-2e-3-0.99-v0', 'CustomAdamW 0.99', cmap(0.), -1),
        #(f'oldcustomadamw-ViT-B-32-{4096*4}-2e-3-0.99-v0', 'CustomAdamW 0.99', cmap(0.5), -1),
        
        (f'customadamw-ViT-B-32-{4096*4}-2e-3-0.95-v0', 'CustomAdamW 0.9','k', 2400),
        # (f'customadamw-ViT-B-32-{4096*4}-2e-3-0.99-v0', 'CustomAdamW 0.99', cmap(1.), -1),
        #(f'clipadamw-ViT-B-32-{4096*4}-2e-3-0.995-v0', 'clipAdamW 0.99', cmap(0.5), -1),
        #(f'customadamw-ViT-B-32-{4096*4}-2e-3-0.995-v0', 'CustomAdamW 0.99', cmap(1.), -1),
        #
        #(f'customadamw-ViT-B-32-{4096*4}-2e-3-0.9-v0', 'CustomAdamW 0.99', cmap(0.5), -1),
        #(f'clipadamw-ViT-B-32-{4096*4}-2e-3-0.999-v0', 'CustomAdamW 0.9', cmap(0.), -1),
        
        (f'stableadamw-1-ViT-B-32-{4096*4}-2e-3-0.4-v0', 'CustomAdamW 0.9', cmap(0.), -1),
        (f'stableadamw-1-ViT-B-32-{4096*4}-2e-3-0.333-v0', 'CustomAdamW 0.9', cmap(1./3), -1),
        (f'stableadamw-1-ViT-B-32-{4096*4}-2e-3-0.25-v0', 'CustomAdamW 0.9', cmap(2./3), -1),
        #(f'stableadamw-1-ViT-B-32-{4096*4}-2e-3-0.2-v0', 'CustomAdamW 0.9', cmap(2./3), -1),
        (f'stableadamw-1-ViT-B-32-{4096*4}-2e-3-0.15-v0', 'CustomAdamW 0.9', cmap(1.), -1),
        #(f'stableadamw-1-ViT-B-32-{4096*4}-2e-3-12-v0', 'CustomAdamW 0.9', cmap(1.), -1),
        #(f'customadamw-ViT-B-32-{4096*4}-2e-3-0.99-v0', 'CustomAdamW 0.99', cmap(1.), -1),
    ...
    for j, (file, name, color, lim) in enumerate(file_list):
        ...
        if log_level >= 1:
            ...
            for i in range(1):
                ...
                #df = df[df[0] > 30000]
                #ax.set_yscale('log')
                ax.plot(df.iloc[:, 0], np.minimum(min_loss, df.iloc[:, 1]), color=color)#, alpha=0.5)#, label='beta2 = 0.99' if j ==0 else 'beta2 = 0.9')#, alpha=0.3)#, label=name)# alpha=0.5,
        ...
        for jj, module in enumerate(modules):
            ...
            for i in range(1):
                ...
                idx = 22
                #if jj == 0:
                ax.plot(df.iloc[:, 0], 1-df.iloc[:, idx], color=color, alpha=alpha)
    ...
    plt.savefig('/admin/home-mitchellw/forks/open_clip_fork/plots/loss_plot_b2.png', bbox_inches='tight')
