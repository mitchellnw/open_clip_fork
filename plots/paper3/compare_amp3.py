...
if __name__ == '__main__':
    ...
    fig = plt.figure(figsize=(12, 3 * 2))#, layout='tight')
    ...
    for k, (model, d) in enumerate([
        ...
        for template, name, color, marker in [
            ...
            if d == 14:
                ax.plot(tdf.iloc[:, 0], np.sqrt(tdf.iloc[:, d]), color=color,alpha=1, label=name, linewidth=1.2)
            else:
                ax.plot(tdf.iloc[:, 0], tdf.iloc[:, d], color=color,alpha=1, label=name, linewidth=1.2)#, marker='o')#, alpha=0.5)#, label='beta2 = 0.99' if j ==0 else 'beta2 = 0.9')#, alpha=0.3)#, label=name)# alpha=0.5,
            badness = 0
            if d == 6:
                for axv in tdf[np.isinf(tdf[6])][0].values:
                    ax.axvline(axv, color='gray', linestyle='--', alpha=0.75, linewidth=1)
                    print(axv)
                    badness += 1
            ...
            for m in [2123, 2487, 2726, 2968]:
                if template == 'opt/customadamw-ViT-H-14-16384-2e-3-0.98-v1':
                    ...
                    ax.scatter(m, fn(mdf[d].values[-1]), s=40, facecolors='C6', edgecolors='C9', linewidth=1, zorder=10)
