...
if __name__ == '__main__':
    ...
    for k, model in enumerate(['H-14']):
        ...
        to_enum = [
            ('clipadamw-ViT-B-32-16384-5e-4-0.99-25-v0', 'CosineLR, Epochs=25, LR=5e-4',cmap(0.), -1),
            ('clipadamw-ViT-B-32-16384-1e-3-0.99-25-v0', 'CosineLR, Epochs=25, LR=1e-3',cmap(0.5), -1),
            ('clipadamw-ViT-B-32-16384-2e-3-0.99-25-v0', 'CosineLR, Epochs=25, LR=2e-3',cmap(1.), -1),
        ]
        ...
        for template, name, color, marker in reversed(to_enum):
            ...
            for ep in range(100):
                if not os.path.exists(f'/fsx/home-mitchellw/experimetns/opt4/{newtemp}/checkpoints/eval_epoch_{ep}.pt'):
                    continue
                fname = f'/fsx/home-mitchellw/experimetns/opt4/{newtemp}/checkpoints/eval_epoch_{ep}.pt'
        ...
        for template, name, color, marker in reversed(to_enum):
            ...
            newtemp = template#.format(model)
            for ep in range(100):
                if not os.path.exists(f'/fsx/home-mitchellw/experimetns/opt4/{newtemp}/checkpoints/eval_ema_1_{ep}.pt'):
                    continue
                fname = f'/fsx/home-mitchellw/experimetns/opt4/{newtemp}/checkpoints/eval_ema_1_{ep}.pt'
                top1 = get_metrics(fname)
                if top1 < 0.5:
                    continue
    ...
    leg = ax.legend(fontsize=10,  bbox_to_anchor=(0.5, -0.3),)
    ax.set_title('ViT-Huge', fontsize=10)
    fig.subplots_adjust(
        top=0.95, left=0.07, right=0.9, bottom=0.3, wspace=0.32, hspace=0.28
    )

    plt.savefig('/admin/home-mitchellw/forks/open_clip_fork/plots/averaging/avg_res.pdf', bbox_inches='tight')
