...
if __name__ == '__main__':
    ...
    file_list = [
        ...
        #('clip-H-14-pd05-bs32k-w8k-opt1e-3-09-098-amp_bfloat16-cinit-v1', 'c-init (lr=1e-3, beta2=0.98, warmup=8k, bs=32k)', 'C3', -1)
    ]
    ...
    for j, (file, name, color, lim) in enumerate(file_list):
        ...
        if log_level >= 1:
            ...
            for i in range(1):
                ...
                df = proc(df, lim)
                ax.plot(df.iloc[:, 0], np.minimum(min_loss, df.iloc[:, 1]), color=color, label=name)# alpha=0.5,
