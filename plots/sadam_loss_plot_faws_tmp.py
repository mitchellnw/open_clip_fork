...
if __name__ == '__main__':
    ...
    file_list = [
        ...
        (f'cadamw5cat-ViT-B-16-pd05-{4096*4}-2e-3-0.95-v0', 'CAdamW 0.95', cmap(0.5), -1),
        (f'cadamw5cat-ViT-B-16-pd05-{4096*4}-2e-3-0.9-v0', 'CAdamW 0.9', cmap(0.25), -1),
    ...
    for j, (file, name, color, lim) in enumerate(file_list):
        ...
        for jj, module in enumerate(modules):
            ...
            for i in range(1):
                ...
                df = pd.read_csv(f'/fsx-scaling/mitchellw/experiments/open_clip_b2/{file}/data/{i}/{layer}', names=list(range(17+5+4)))
                df = proc(df, lim)
    ...
    plt.savefig('/fsx-labs/mitchellw/open_clip_fork/plots/b2_loss_plot_faws_tmp.png', bbox_inches='tight')
