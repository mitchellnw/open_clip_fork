...
def cast_and_quantize(x: torch.Tensor):
    ...
    _cast_and_quantize[grid](x, output, output_maxs, n_elements, BLOCK_SIZE=x.shape[1], P2=P2)
    return output, output_maxs

# write a triton kernel for transpose
...
def cast_global(x: torch.Tensor):
    ...
    n_elements = output.numel()
    grid = lambda meta: (triton.cdiv(n_elements, meta['BLOCK_SIZE']),)
    _cast_global[grid](x, absmax, output, n_elements,  BLOCK_SIZE=P2)
...
def test_all():
    ...
    w = 0.01 * torch.randn(d, d, device='cuda', dtype=torch.float32)
    wt = w.t()
    # ################## -1. triton cast ###################
    from transpose import transpose_and_global_quantize
    for _ in range(8):
        w_out, w_max = transpose_and_global_quantize(w)
    ...
    w_out = torch.transpose(w_out.clone(), 0, 1)
    ...
    def fp16_matmul(x, wt):
        with torch.cuda.amp.autocast(dtype=torch.float16):
            torch.matmul(x, wt)

    ################### 2. torch matmul fp16 ###################
    for _ in range(8):
        th_c = fp16_matmul(x, wt)
