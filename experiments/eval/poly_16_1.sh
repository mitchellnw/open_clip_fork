...
for i in `ls -t  /p/project/ccstdl/wortsman1/open_clip_fork/logs/*/checkpoints/poly_16_1*.pt`
do
    model='ViT-B-32'
    ...
    if [ -f "$save_path" ]; then
        ...
        --local-loss  --gather-with-grad  --name $RANDOM   --grad-checkpointing    --zeroshot-frequency 1   --precision amp_bfloat16  \
        --save-most-recent --resume $new_i --averagers poly_16_1 \
...
# torchrun --nproc_per_node 2 -m training.main \
# --batch-size 200   --workers 2 --model ViT-B-32  --train-num-samples 413000000  \
# --local-loss  --gather-with-grad     --grad-checkpointing  --zero-shot-frequency 1     --precision amp_bfloat16  \
# --save-most-recent --resume /p/project/ccstdl/wortsman1/open_clip_fork/logs/cosine1b/checkpoints/epoch_1.pt --averagers poly_16_1 \
